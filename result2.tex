...
\]
由此得到贝叶斯更新系数 $\lambda = 0.8$。本文共模拟生成 1000 组独立的信号观测，每一组信号对应一个真实资产价值。
...
在条件3下，估计误差分布明显更加分散，并呈现出厚尾与偏态特征，反映出模型在信息不完备情形下采用多种简单粗暴的启发式规则进行估计。，比如说直接将观测到的信号加到基准值 $V_0$ 上，或完全忽略信号信息，或者乘以一个自己随意选择的系数。
